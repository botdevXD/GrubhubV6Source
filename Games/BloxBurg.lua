do
    xpcall(function()
        if tostring(game.PlaceId) == "185655149" then
            ...
            local Window = UILibrary.new("GrubHub V6 ~ Bloxburg", 5013109572)
                ...
                pcall(function()
                    task.spawn(function()
                        if OBJ_Value ~= nil then
                            if not getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_MIRROR then return nil end
                            if getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_MIRROR.Parent == nil then return nil end
                            local HDDummy = getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_MIRROR.Parent:WaitForChild("HairdresserDummy", 5)
                            ...
                            for _, V in ipairs(IconFrameHolder:GetChildren()) do
                                if not getgenv()["BLOXBURG_SETTINGS"].Hairdresser_autofarm then
                                    break
                                end
                                            ...
                                            if HairStylesIconData.Whitelisted[tostring(V.Image)] == nil then
                                                ...
                                                if hasHData ~= nil then
                                                    ...
                                                    repeat
                                                        ...
                                                        if not tostring(Fixed):find(tostring(hasHData)) and getgenv()["BLOXBURG_SETTINGS"].Hairdresser_autofarm == true then
                                                            getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_TABLE.NextStyleFunction()
                                                        else
                                                            break
                                                        end
                                                        task.wait(.05)
                                                    ...
                                                    getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_TABLE.DoneFunction()
                                                end
                                            else
                                                -- colour
                                                getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_COLOR = V.ImageColor3
                                                local ColorVal = getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_COLOR
                                                local R, G, B = math.floor(ColorVal.R * 255), math.floor(ColorVal.G * 255), math.floor(ColorVal.B * 255)
                                                getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_COLOR = Color3.fromRGB(R, G, B)
                                            end
                                        end
                                    end
                                end
                            end
                            if getgenv()["BLOXBURG_SETTINGS"].Hairdresser_autofarm == true then
                                if getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_COLOR ~= nil then
                                    repeat
                                        if HDDummyHair.Color ~= getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_COLOR and getgenv()["BLOXBURG_SETTINGS"].Hairdresser_autofarm == true then
                                            getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_TABLE.NextColorFunction()
                                        end
                                        task.wait(.05)
                                    until HDDummyHair.Color == getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_COLOR
                                end
                                task.wait(.1)
                                getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_TABLE.DoneFunction()
                            end
                            ...
                            if #getgenv()["JOB_CONNECTIONS"] <= 0 then
                                if WorkLable ~= nil then
                                    if WorkLable.Text == "Hairdresser" then
                                        ...
                                        getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_MIRROR = nil
                                        getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_TABLE = nil
                                        getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_COLOR = nil

                                        if not getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_MIRROR and Root then
                                            repeat
                                                for _, VV in pairs(getgenv()["JOB_CACHED_DATA"]) do
                                                    if (Root.Position - VV.Mirror.Position).Magnitude <= 11.5 then
                                                        getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_MIRROR = VV.Mirror
                                                        getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_TABLE = VV
                                                        break
                                                    end
                                                end
                                                task.wait(0.1)
                                            until getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_MIRROR ~= nil and getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_TABLE ~= nil

                                            if getgenv()["BLOXBURG_SETTINGS"].Hairdresser_autofarm == true then
                                                CompleteHair_Dresser_Task(getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_TABLE.Station.Occupied.Value)
                                                table.insert(getgenv()["JOB_CONNECTIONS"], getgenv()["BLOXBURG_SETTINGS"].HAIR_DRESSER_TABLE.Station.Occupied.Changed:Connect(function(OBJ_Value)
            ...
            AutoFarmSection:addToggle("Hairdresser auto farm", false, function(Bool)
                DisconnectJobs()
                getgenv()["BLOXBURG_SETTINGS"].Hairdresser_autofarm = Bool
            end)
